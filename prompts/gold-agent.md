...
"Rounding favours the user by X wei per operation, accumulating to Y tokens across
...
4. Do NOT flag anything listed in KNOWN_ISSUES.md as an accepted risk.
5. If you rate something below Medium, explain why it is NOT profitable. The burden of proof is on dismissal.
...
Look at `audit-workspace/recon/math-operations.json` — every arithmetic operation has been inventoried.
...
1. `AUDIT_CONTEXT.md` — economic parameters (fee rates, pool mechanics, token amounts)
2. `PROPERTIES.md` — economic invariants you must verify
3. `KNOWN_ISSUES.md` — accepted risks (do not flag these)
4. `ATTACK_PATTERNS.md` — AP-1 (rounding exploitation), AP-10 (first-depositor), AP-11 (balance-delta)
...
8. `audit-workspace/recon/balance-delta-patterns.json` — contracts using balanceOf(address(this)) differentials

## Required Skill Invocations

Before diving into manual arithmetic analysis:

1. **Run `/tob-scv-scan`** focusing on arithmetic-related vulnerability classes
...
- Token↔share conversion math in any pool or vault contracts
- Fee collection and distribution functions
- Any ERC4626 vault integration (convertToAssets / convertToShares rounding)
...
```json
{
  ...
  "economic_analysis": {
    ...
    "at_1k_operations_tokens": 0.0,
    "at_10k_operations_tokens": 0.0,
    "at_100k_operations_tokens": 0.0,
    "gas_cost_per_op_usd": 0.0,
...
Rounding bugs in DeFi have paid six-figure bounties. The question is not WHETHER there
are rounding issues in this protocol, but whether they are PROFITABLE to exploit at scale.
